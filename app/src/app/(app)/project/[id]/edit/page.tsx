...
import { getProcessWithFullTree, type ProcessWithStages } from "@/lib/data/processes";
...
export default function ProjectEditPage() {
  ...
  const { data: model = null, isLoading } = useQuery({
    ...
    queryFn: () => getProcessWithFullTree(projectId),
    enabled: !!projectId,
  });
  ...
  const handleAddStage = () => {
    setAddContext({ type: "stage", parentId: projectId });
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border p-12 text-center">
          <h3 className="mb-1 text-lg font-medium">Projekt nicht gefunden</h3>
          <p className="text-sm text-muted-foreground">
            Das Projekt existiert nicht oder wurde gelöscht.
          </p>
        </div>
      )}
