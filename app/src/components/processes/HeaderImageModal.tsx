...
export function HeaderImageModal({
  ...
  }, [processId, processName, processDescription, userPrompt]);

  const handleApply = useCallback(async () => {
    if (!generatedPath) return;
    try {
      await updateProcess(processId, { header_image: generatedPath });
    ...
    setUserPrompt("");
  }, [generatedPath, processId, onImageGenerated, onOpenChange]);
      ...
      <DialogContent className="sm:max-w-[640px] max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            KI-generiertes Headerbild für den Prozess
          </DialogDescription>
        </DialogHeader>
